...
const Breadcrumbs = ({ location }) => {
  ...
  const allPages = query.newsPosts.nodes
    .map(page => (
      { slug: `/news${page.fields.slug}`, title: page.frontmatter.title }
    ))
    .concat(query.genericPages.nodes
      .map(page => (
        { slug: page.fields.slug, title: page.frontmatter.title }
      ))
    )
    .concat(query.site.siteMetadata.menuLinks)
    .filter(page => !page.slug.startsWith('http'))
    .map(page => (
      page.slug.endsWith('/') ? page :
        { slug: `${page.slug}/`, ...page }
    ));
  ...
  const pathSlugs = pathComponents.map((component, index) => {
    return '/' + pathComponents.slice(0, index + 1).reduce((previous, current) => {
      return previous + '/' + current;
    }) + '/';
  });
  
  const pageComponents = pathSlugs.map(slug => {
